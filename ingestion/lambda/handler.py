...
def handler(event, context):
    ...
    if today >= RETIRE_ON_OR_AFTER:
        ...
        print(f"[{iso(datetime.now(timezone.utc))}] {msg}")
    ...
    print(f"[{iso(datetime.now(timezone.utc))}] generated {len(events)} events across {TRANSACTION_COUNT} transactions")
    ...
    if failed_days:
        msg = f"done with errors — {len(by_day) - len(failed_days)}/{len(by_day)} partition(s) uploaded; failed: {', '.join(failed_days)}"
        print(f"[{iso(datetime.now(timezone.utc))}] {msg}")
